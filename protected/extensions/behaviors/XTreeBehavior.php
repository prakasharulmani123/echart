...
class XTreeBehavior extends CActiveRecordBehavior {
     ...
     */
    public function getTreeItems($id = null, $showRoot = true, $organize_chart = false) {
        $owner = $this->getOwner();
        $rootId = ($id === null) ? $this->getRootId() : $id;
        $items = array();
        if ($showRoot === false) {
            $models = $owner->findAll(array(
                'condition' => "{$this->parent_id} = '{$rootId}' AND is_personal_staff <> '1'",
                'order' => $this->sort
            ));
            if ($models === null)
                throw new CException('The requested tree does not exist.');
            foreach ($models as $model)
                $items[] = $model->getTreeSubItems($organize_chart);
        }
        else {
            $model = $owner->findByPk($rootId);
            if ($model === null)
     ...
     */
    protected function getTreeSubItems($organize_chart = false) {
        $owner = $this->getOwner();
        $subItems = array();
    ...
    protected function formatTreeItem($model,$organize_chart = false) {
        ...
        if (!is_null($model->userProfile->profPersonalStaff) && !isset($_GET['manager']) && !isset($_GET['organization']) && $organize_chart == true) {
            ...
            $text .= '<adjunct>' . '<a href="javascript:popup(' . $assistant->user_id . ')">' . $assistant->userProfile->prof_firstname . '</a></adjunct>';
        }
